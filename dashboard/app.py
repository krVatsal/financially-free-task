...
if not os.path.exists(db_path):
    ...
    for fname, tname in [("four_wheeler_data.csv", "four_wheeler"), ("three_wheeler_data.csv", "three_wheeler")]:
        fpath = os.path.join(base, fname)
        if os.path.exists(fpath):
            load_csv_to_sqlite(fpath, db_path, tname)

# Vehicle type selector
vehicle_type = st.sidebar.selectbox("Select Vehicle Type", ["Four Wheeler", "Three Wheeler"], index=0)
table_map = {
    "Four Wheeler": {
        "table": "four_wheeler",
    ...
    # Add Two Wheeler here when available
}

meta = table_map[vehicle_type]
conn = create_connection(db_path)
years = pd.read_sql_query(f"SELECT DISTINCT Year FROM {meta['table']}", conn)['Year'].tolist()
...
with tab1:
    ...
    st.download_button(f"Download Filtered Data as CSV", df.to_csv(index=False), f"filtered_{vehicle_type.lower().replace(' ', '_')}.csv")
...
with tab2:
    ...
    # Additional investor metrics (extensible for all vehicle types)
    st.markdown("---")
